...
# 3. MODEL EĞİTİMİ (SMOTE + LINEAR REGRESSION)
...
def evaluate_model(model, X_test, y_test):
    ...
    y_pred = np.clip(y_pred, np.min(y_test), np.max(y_test))
...
# 5. MODEL KAYDETME
def save_model(model, filename='best_linear_regression_model.pkl'):
    joblib.dump(model, filename)
    print(f"\nModel '{filename}' olarak kaydedildi.")


# Özelliklerin dağılımını görselleştirmek
def plot_feature_distributions(X):
    print("\nÖzelliklerin dağılımları görselleştiriliyor...")
    X_df = pd.DataFrame(X, columns=[f'Feature {i + 1}' for i in range(X.shape[1])])

    X_df.hist(figsize=(12, 10), bins=30)
    plt.suptitle('Özelliklerin Dağılımları')
    plt.show()


# Öğrenme eğrisini çizmek
